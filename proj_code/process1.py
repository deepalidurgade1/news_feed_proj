...
class GetRssFeed:
    ...
    # default constructor : used to initialize the parameters while creating object
    def __init__(self):
    # create connection string = 'postgresql://<user>:<password>@<host>:<port>/<db>'
        self.conn_string = 'postgresql://postgres_admin:postgres123@postgres-db-identifier1.cxfegqfzlnwt.ap-south-1.' \
                  'rds.amazonaws.com:5432/db_news_feed'
        # self.conn_string = x
        self.engine = create_engine(self.conn_string)                          # forms a connection to the PostgresSQL database

    # Function to read Table1
    def read_table1(self):
        list1 = []
        print("--------------Reading Table1---------------")
        with self.engine.connect() as connection:                         # using "with" don't require closing of connection
    ...
    def get_rss_feed(self, news_url):
        feed_links = []
    ...
    def read_main(self):
        ...
        list1 = self.read_table1()

        for i in list1:
            df = self.get_rss_feed(i)                        # 2) get rss feed urls from given url extracted from table1
            self.write_to_table2(df)                         # 3) write the Dataframe to table2
        self.engine.dispose()                                # close all connections of the connection pool.
